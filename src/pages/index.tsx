...
const Home: NextPage = () => {
  ...
  const getQuestions = async () => {
    const res = await $api.get(`question/getAll?limit=${questions.length + 5 ?? 5}`);
          ...
          <div className="h-full max-w-[630px] mx-auto p-0 sm:pt-[50px]">
            ...
            {questionsLoadingStatus === "success" && (
              <>
                ...
                <Button onClick={getQuestions} className="bg-white rounded-[10px] text-[17px] text-[#4971ff] font-nunito font-bold py-[7px] shadow-standart w-full mb-[80px] sm:mb-[15px] mt-[20px]">
                  Загрузить еще
                </Button>
              </>
            )}
